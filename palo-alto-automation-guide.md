...
## 11) Starter Automation Script Added: `playbooks/backup.yml`

A first backup playbook is now included at `playbooks/backup.yml`.

### What it does

- Creates a local backup directory
- Exports running configuration XML
- Optionally exports device-state archive
...
ansible-playbook playbooks/backup.yml \
  -e panos_hostname=fw01.example.com \
  -e panos_username=automation-user \
  -e panos_password='REDACTED'
```

### Recommended production usage

- Store credentials in Ansible Vault or secret manager (not CLI args)
- Run from CI/AWX on a schedule (nightly + pre/post change)
- Upload generated artifacts + checksum manifest to immutable object storage

---

## 12) Real-Time Testing Options (PA-VM + Local Mock Lab)

You have two practical ways to test in real time:

### Option A (closest to production): PA-VM lab

Use a licensed/evaluation PA-VM in EVE-NG, VMware, KVM, or cloud.

- Pros: Real PAN-OS behavior (best validation for commits/policies)
- Cons: Requires PA-VM image/license and more setup

### Option B (fast local iteration): Mock PAN-OS API container

This repo now includes a local mock PAN-OS XML API service and a backup test harness.
...
#### Run a real-time backup test against the mock API

```bash
python3 -m venv .venv
source .venv/bin/activate
pip install -r scripts/requirements.txt
python scripts/realtime_backup_test.py --include-device-state
```

This will produce timestamped artifacts under `artifacts/backups/` and a SHA256 manifest.
...
- The mock lab is intended for workflow testing (backup pipeline logic), not PAN-OS feature fidelity.
- For final pre-production validation, run the same workflow against a real PA-VM or non-prod firewall.

---

## 13) RHEL 9 Python Bundle for Cisco + Palo Alto Automation

If you want a transferable environment package for a RHEL 9 host running Python 3.9, use:

- `requirements/network-automation-rhel9.txt`
- `scripts/build_rhel9_automation_bundle.sh`
- `docs/rhel9-network-automation-bundle.md`
